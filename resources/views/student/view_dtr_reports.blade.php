...
@section('body')
    ...
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Submitted Monthly Daily Time Records Reports</h5>
            <ul class="list-group">
                @foreach($dtrReports as $report)
                    <li class="list-group-item">
                        {{ \Carbon\Carbon::parse($report->month_year)->format('F Y') }}
                        <button onclick="showPreview('{{ route('internship.files.preview', ['type' => 'dtr', 'id' => $report->id]) }}')" class="btn btn-dark btn-sm float-end">Preview</button>
                    </li>
                @endforeach
            </ul>
        </div>
